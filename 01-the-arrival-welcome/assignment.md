...
id: ermykdy1tbse
type: challenge
title: "<span id="assignment.7" lang="en" hist="sky-telco">📡 Chapter 1: The Arrival</span>"
teaser: <span id="assignment.8" lang="en" hist="sky-telco">Skyline Telecom is drowning in legacy <span id="ch1.intro1.1" lang="nolang" no>hypervisor</span> costs. Step into the network operations center, take command of SUSE Virtualization, and inspect your new command center.</span>
notes:
- type: text
  contents: |
    <span id="assignment.1" lang="en" no># Welcome to the <span id="assignment.1.1"  lang="nolang" no>SUSE Virtualization Rodeo!</span>

    Please wait while we prepare your lab environment.</span><span lang="en" id="ch1.waiting1" hist="sky-telco">The storm-alert wall is flashing across every screen in Skyline Telecom's network operations center...
    Sarah, the CTO, is waiting for you in the war room.</span>
...
<span id="assignment.9" lang="en" hist="sky-telco">📡 Chapter 1: The Arrival</span>
==========================
...
<span lang="en" id="ch1.intro1" hist="sky-telco">The alert wall in the Skyline Telecom network operations center pulsed an angry, unbroken red, each blinking icon marking another cell site or core router flirting with capacity. Inside the glass-walled war room, the atmosphere was just as charged. Sarah, the Chief Technology Officer, paced the length of the room, her eyes fixed on a massive overhead monitor projecting a sea of dropped-call warnings and latency spikes.

She turned to you, her voice tight with exhaustion. *"We are losing precious milliseconds on every single call handoff and data session. Our legacy <span id="ch1.intro1.1" lang="nolang" no>hypervisor</span>s are buckling under the sheer volume of modern 5G and streaming traffic. The infrastructure is brittle, the storage arrays are constantly falling out of synchronization, and our licensing costs are draining our engineering budget completely dry. We cannot survive another year chained to these monolithic, antiquated systems."*

You sit quietly at the end of the conference table, reviewing the network schematics she handed you. As an elite **Infrastructure Architect**, you have been brought in for one specific purpose: to save Skyline Telecom from total operational gridlock. They need a bridge to the cloud-native world without rebuilding their entire service stack from scratch.

*"We have a plan, Sarah,"* you finally say, closing your laptop with a reassuring click. *"We are going to transition the entire datacenter to <span id="ch1.intro1.2" lang="nolang" no>SUSE Virtualization</span>. We will bring your legacy systems into the modern era, and we will do it without dropping a single call."*</span>
</div>


<span id="assignment.2" lang=en no>Your journey begins right now. Before you can begin dismantling the old world, you need to establish a foothold in the new one and dig into the environment.



## What is <span id="ch1.intro1.2" lang="nolang" no>SUSE Virtualization</span>?

<span id="ch1.intro1.2" lang="nolang" no>SUSE Virtualization</span> (also known as **<span id="assignment.2.1" lang="nolang" no>Harvester</span>**) is a modern, open-source hyperconverged infrastructure (HCI) platform built on <span id="assignment.2.2" lang="nolang" no>Kubernetes</span>. It runs directly on bare metal and gives the operator enterprise-grade **virtual machines** on a cloud-native foundation, <span id="ch1.intro2"  lang="en" hist="sky-telco">exactly the bridge Skyline Telecom needs</span>:

- **<span id="assignment.2.3" lang="nolang" no>KubeVirt</span> + <span id="assignment.2.4" lang="nolang" no>KVM</span>/<span id="assignment.2.5" lang="nolang" no>QEMU</span>**: enterprise virtualization as native <span id="assignment.2.2" lang="nolang" no>Kubernetes</span> workloads. Underneath sits the same battle-hardened **<span id="assignment.2.4" lang="nolang" no>KVM</span>/<span id="assignment.2.5" lang="nolang" no>QEMU</span>** pair that has powered <span id="assignment.2.6" lang="nolang" no>Linux</span> virtualization for decades, which is why the platform can run a huge variety of guest operating systems, <span id="ch1.intro3"  lang="en" hist="sky-telco">including the very old ones still serving in Skyline Telecom's dustiest legacy corners, patiently waiting for their migration</span>
- **<span id="assignment.2.7" lang="nolang" no>SUSE Storage</span> (<span id="assignment.2.8" lang="nolang" no>Longhorn</span>)**: distributed, replicated block storage across every node, set up and ready out of the box. <span id="ch1.intro4"  lang="en" hist="sky-telco">And if the operator ever prefers different storage</span>, **any <span id="assignment.2.9" lang="nolang" no>CSI</span>-compatible storage driver plugs right in**, freedom of choice, never lock-in
...
Because the platform runs *on* <span id="assignment.2.2" lang="nolang" no>Kubernetes</span>, containerized workloads can run on the very same cluster. Keep the division of labor straight from day one: the <span id="ch1.intro1.2" lang="nolang" no>SUSE Virtualization</span> UI manages **virtual machines**; managing containers (and managing whole fleets of clusters) is the job of <span id="assignment.2.12" lang="nolang" no>**Rancher Prime**</span>, which you will meet in a moment.

<span id="ch1.intro5"  lang="en" hist="sky-telco">Every proprietary component bleeding Skyline Telecom's budget dry has a modern, open-source replacement:</span>
...
| Proprietary storage array |  SUSE storage, or any <span id="assignment.2.9" lang="nolang" no>CSI</span> driver <span id="ch1.intro6"  lang="en" hist="sky-telco">the operator chooses</span> |
...
No vendor lock-in. No virtualization tax. No proprietary kernel. **One platform, one bill**, <span id="ch1.intro7"  lang="en" hist="sky-telco">exactly what you promised Sarah in the war room</span>.
...
<span id="ch1.intro8"  lang="en" hist="sky-telco">> [!NOTE]
> Disclaimer: This lab is meant to be educational and not to provide instructions on how to configure a production environment for a 'telecom provider', most decisions made are with the limitations and purpose of this environment.</span>
...
For your records, your Architect Credentials are as follows:</span>
...
<span id="assignment.3" lang=en> [!NOTE]
...
The platform can also be connected to **Rancher Prime**, <span id="ch1.task2a"  lang="en" hist="sky-telco">and it is important to understand who does what in the operator's new world:</span>

- <span id="ch1.intro1.2" lang="nolang" no>SUSE Virtualization</span> manages the **virtual machines** on this cluster.
- **Rancher Prime** manages **many clusters at once** (every <span id="ch1.intro1.2" lang="nolang" no>SUSE Virtualization</span> cluster in every regional data center), plus centralized **users, roles, and access control (<span id="assignment.6.13" lang="nolang" no>RBAC</span>)**, and the **container workloads** the operator will run alongside its VMs.
...
5. Below it you can see the registration instructions. Follow them, and remember to select **<span id="assignment.6.14" lang="nolang" no>Insecure Skip TLS Verify</span>** when editing the cluster-registration-url setting. This should be done in [button label="SUSE Virtualization UI" variant="success"](tab-0).
...
<span id="ch1.bonus1a"  lang="en" hist="sky-telco">A healthy storage backend is key for telecom operations</span>. <span id="ch1.intro1.2" lang="nolang" no>SUSE Virtualization</span> uses **<span id="assignment.2.7" lang="nolang" no>SUSE Storage</span>** to replicate every volume across the cluster.
...
- **Confirm the exact platform version the operator is running:**
...
- **Fleet management and <span id="assignment.6.13" lang="nolang" no>RBAC</span> included.** Rancher Prime is <span id="ch1.why1"  lang="en" hist="sky-telco">ready to command every cluster the operator will ever run, with one login and one set of access rules.</span>
