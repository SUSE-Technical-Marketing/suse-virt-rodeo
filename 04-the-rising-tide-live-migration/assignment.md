...
id: xjv2r0tfyztq
type: challenge
title: "<span lang="en" hist="vertrex-bank">\U0001F30A Chapter 4: The Rising Tide</span>"
...
In the </span> [button label="SUSE Virtualization UI" variant="success"](tab-0) <span lang="en" no>, go to <span lang="nolang" no>**Virtual Machines**</span>:
...
3. Select <span lang="nolang" no>**Pause**</span> then click <span lang="nolang" no>**Apply**</span> after prompted to confirm.
4. Wait for its state to change to <span lang="nolang" no>**Paused**</span>
...
Switch to the </span> [button label="Cluster Terminal" variant="success"](tab-1) <span lang="en" no>. You need a continuous heartbeat monitor to prove the network connection remains unbroken during the evacuation.

Start the heartbeat monitor from <span lang="nolang" no><b class="highlightcopy">webserver-prod</b></span>:
...
> [!IMPORTANT]
> Leave the ping running continuously in the terminal. **Do not stop it.** This scrolling stream of replies is your proof of zero downtime. Switch your focus back to the <span lang="nolang" no>SUSE Virtualization</span> UI.
</span>
...
In the </span> [button label="SUSE Virtualization UI" variant="success"](tab-0) <span lang="en" no>, go to **<span lang="nolang" no>Virtual Machines</span>** and locate the following instance:
</span>
...
<span lang="en" no>
1. Read the <span lang="nolang" no>**Node**</span> column and **write down** which node the gateway is running on: you will want proof it moved
...
3. Select <span lang="nolang" no>**Migrate**</span> from the context menu
4. Choose a different, safe target node from the dropdown list
5. Click <span lang="nolang" no>**Apply**</span>

Behind the scenes, <span lang="nolang" no>KubeVirt</span> copies the VM's live memory pages to the target node over the network, tracking and re-copying any pages the busy gateway dirties mid-flight, until it can freeze, flip, and resume execution on the new node in a fraction of a second.
</span>
...
Immediately switch back to the </span> [button label="Cluster Terminal" variant="success"](tab-1) <span lang="en" no> tab and watch the ping sequence.
</span>

<div id="402" class="story">

<span lang="en" hist="vertrex-bank">
You hold your breath as the <span lang="nolang" no>hypervisor</span> coordinates the massive memory transfer over the network. The pings continue scrolling down the screen, **completely uninterrupted**. The virtual machine seamlessly materializes on the new physical node just as sparks begin to fly from the water-damaged chassis in Rack 4.
</span>
...
<span lang="en" hist="vertrex-bank">You exhale sharply. The transaction flow survived.</span>
</div>
...
Back in the </span> [button label="SUSE Virtualization UI" variant="success"](tab-0) <span lang="en" no>, the <span lang="nolang" no>**Node**</span> column for <b class="highlightcopy">webserver-prod</b> now shows a **different node** than the one you wrote down. </span><span lang="en" hist="vertrex-bank">The gateway physically moved while its customers never noticed.</span>

<div id="404" class="story">
<span lang="en" hist="vertrex-bank">Now produce the evidence Sarah will forward to the regulators — the guest's uptime counter never reset, meaning the operating system never stopped running:</span>
</div>
...
Return to the </span> [button label="SUSE Virtualization UI" variant="success"](tab-0) <span lang="en" no>. Locate the virtual machine you paused earlier:
</span>
...
2. Select <span lang="nolang" no>**Unpause**</span> to allow the non-critical jobs to resume
...
<span lang="en" hist="vertrex-bank">The gateway is safe — but the water-damaged node is still dripping, and smaller workloads may still be running on it. You are not going to migrate them one by one while a puddle spreads across the floor. Let the platform manage it.</span>
</div>


<span lang="en" no>
In the </span> [button label="SUSE Virtualization UI" variant="success"](tab-0) <span lang="en" no>, go to <span lang="nolang" no>**Hosts**</span>:

1. Find the node that <b class="highlightcopy">webserver-prod</b> was running on **before** the migration, the one you wrote down.
</span>

<i id="406" class="story"><span lang="en" hist="vertrex-bank">That is the water-damaged machine</span></i>

<span lang="en" no>
2. Click the <img class="embedded_img" desc="three vertical dots" src="../assets/three_vertical_dots.png"/> on its row and select <span lang="nolang" no>**Enable Maintenance Mode**</span>, then <span lang="nolang" no>confirm</span>

Now watch the **<span lang="nolang" no>Virtual Machines</span>** page: every VM still living on the damaged node live-migrates off it **automatically**. The platform picks healthy target nodes, moves the workloads one by one, and leaves the node empty. No spreadsheets, no manual target-picking, no forgotten VM.

> [!NOTE]
> This may take some time in this lab environment.

Once the node shows <span lang="nolang" no>**Maintenance**</span> and its VM count reaches zero, the (virtual) repair crew swaps the (virtual) coolant valve. Bring the node back into service:

3. Click the <img class="embedded_img" desc="three vertical dots" src="../assets/three_vertical_dots.png"/> on its row again and select <span lang="nolang" no>**Uncordon**</span> and then <span lang="nolang" no>**Disable Maintenance Mode**</span>
...
New to <span lang="nolang" no>Kubernetes</span>? **Skip ahead freely.** Otherwise: every migration is itself a <span lang="nolang" no>Kubernetes</span> object, which means it is auditable. In the </span> [button label="Cluster Terminal" variant="success"](tab-1) <span lang="en" no>:
...
- **Think ahead:** what happens if a *node* fails without warning, before anyone can migrate? Check each VM's run strategy: <span lang="nolang" no>SUSE Virtualization</span> can reschedule VMs from a failed host automatically:
...
> [!NOTE]
> **Beyond compute:** the same zero-downtime idea also applies to *disks*. <b class="virt"><span lang="nolang" no>SUSE Virtualization</span></b> supports **in-place storage live migration** (moving a running VM's volumes between storage backends, for example from <span lang="nolang" no>Longhorn</span> to an external <span lang="nolang" no>CSI</span> array) without stopping the VM. Compute evacuated tonight, storage evacuated next quarter, and the gateway never notices either one.
</span>
